...
const generatePaidToken = (payload, key) => {
    ...
    if (!access_token || !refresh_token)
        throw new error_response_1.ResponseError({ metadata: 'Lỗi do tạo key' });
...
const verifyAccessToken = ({ user, keyStore, client_id, token, key, req, res, next }) => {
    jsonwebtoken_1.default.verify(token, key, (error, decode) => {
        if (error) {
            ...
            return next(new error_response_1.AuthFailedError({ metadata: 'Token không đúng' }));
        }
        const payload = decode;
        if (payload._id !== client_id)
            return next(new error_response_1.BadRequestError({ metadata: 'Token không thuộc về user' }));
        req.user = user;
...
const verifyRefreshToken = ({ user, keyStore, client_id, token, key, req, res, next }) => {
    ...
    jsonwebtoken_1.default.verify(token, key, (error, decode) => {
        if (error) {
            return next(new error_response_1.ForbiddenError({ metadata: 'Token không đúng' }));
        }
        const payload = decode;
        if (keyStore.refresh_token_used.includes(token)) {
            return next(new error_response_1.ForbiddenError({ metadata: 'Token đã được sử dụng' }));
        }
        if (payload._id !== client_id)
            return next(new error_response_1.BadRequestError({ metadata: 'Token không thuộc về user' }));
        req.user = user;
